...
def main(args):
    ...
    model_string_name = "refine-net"  # e.g., GPT-XL/2 --> GPT-XL-2 (for naming folders)
    ...
    model = refine_net(
        ...
        n_head=12,
        dim=768,
        in_vocab_size=args.in_vocab_size,
    ...
    logger.info(f"refine_net Parameters: {sum(p.numel() for p in model.parameters()):,}")
    ...
    logger.info(f"Loading {args.idx_to_cluster}.")
    idx_to_cluster = torch.load(args.idx_to_cluster, map_location="cpu").to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            cluster_indices = idx_to_cluster[z_indices]
            with torch.amp.autocast("cuda", dtype=ptdtype):
                logits, loss, acc = model(cond_idx=c_indices, idx=cluster_indices, targets=z_indices)
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f}, "
                            f"accuracy: {running_acc/log_steps:.5f}, lr {args.lr:.6f}, "
                            f"ETA: {str(timedelta(seconds=(total_steps-train_steps)/steps_per_sec))}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-layers", type=int, default=1, help="transformer layers num")
    parser.add_argument("--in-vocab-size", type=int, default=8192)
    parser.add_argument("--out-vocab-size", type=int, default=16384)
    ...
    parser.add_argument("--idx-to-cluster", type=str)
